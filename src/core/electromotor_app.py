...
from src.session.errors import NetworkError, APIError, ServerError, NotFoundError
...
class ApplicationElectromotor:
            ...
            for attempt in range(retries):
                async with ElectromotorAPI() as api:
                    try:
                        ...
                        if response:
                            data = await data_extraction_electromotor(response) 
                        else:
                            continue
    ...
    async def start(self):
        ...
        self.logger.info(r'''
         _______  __       _______   ______ .___________..______        ______   .___  ___.   ______   .___________.  ______   .______      
        |   ____||  |     |   ____| /      ||           ||   _  \      /  __  \  |   \/   |  /  __  \  |           | /  __  \  |   _  \     
        |  |__   |  |     |  |__   |  ,----'`---|  |----`|  |_)  |    |  |  |  | |  \  /  | |  |  |  | `---|  |----`|  |  |  | |  |_)  |    
        |   __|  |  |     |   __|  |  |         |  |     |      /     |  |  |  | |  |\/|  | |  |  |  |     |  |     |  |  |  | |      /     
        |  |____ |  `----.|  |____ |  `----.    |  |     |  |\  \----.|  `--'  | |  |  |  | |  `--'  |     |  |     |  `--'  | |  |\  \----.
        |_______||_______||_______| \______|    |__|     | _| `._____| \______/  |__|  |__|  \______/      |__|      \______/  | _| `._____|
