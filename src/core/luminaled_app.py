...
from src.api.luminaled import LuminaledAPI
...
class ApplicationLuminaled:
    ...
    async def start(self):
        ...
        to_parse = self.settings.google.luminaled_index_to_parse
        name_list = f'LUMINALED {to_parse}'
        self.logger.info(f'⏩LUMINALED⏪')
        ...
        for category in to_parse:
            ...
            for name, url in categories.items():
                async with LuminaledAPI() as api:
    ...
    async def _task_html_data(self, api: LuminaledAPI, url: str, count: int) -> None:
    ...
    async def _task_parse_html(self, url: str, response, count: int) -> None:
        ...
        result = await data_extraction_luminaled(response) 
        self.final_data[url] = result
        await asyncio.sleep(0,3)
        self.logger.info(f' {count} Готово -> {url} ✅')

    async def _task_all_products(self, api: LuminaledAPI, url: str) -> None:
        async with self.semaphore:  # Use semaphore here
